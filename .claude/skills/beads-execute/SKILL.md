...
When invoked with a specific bead ID (e.g., `/beads-execute beadsmith-c87.1`):
...
When invoked with an epic ID (e.g., `/beads-loop beadsmith-c87`):
...
- [ ] Remove complete-to-org references — org-mode layer not used in this
  project. Define alternative "mark work done" mechanism if needed.
- [ ] Review opencode-specific commands (session list, export) and
  generalize or remove.
- [ ] Review beads-harness references — verify harness is available or
  document manual execution workflow.

## Attribution

Imported from [beadsmith](https://github.com/user/beadsmith/tree/main/skills/beads-execute).
Light adaptation: org-mode and agent-specific references noted.

...
**Status key:**
- **Vendored**: Copied from upstream with attribution. Safe to update from upstream.
...
- **commit**: Issue references use SourceHut `todo.sr.ht` trailers
  instead of GitHub/Sentry refs. Co-Authored-By uses
  `Claude Opus 4.5 <noreply@anthropic.com>`.
- **find-bugs**: Default branch detection uses `git` instead of `gh`
  (GitHub CLI) since the project is hosted on SourceHut.
- **creating-skills**: Blended from Anthropic's `skill-creator` (information
  architecture, progressive disclosure, degrees of freedom) and obra's
  `writing-skills` (BDD methodology, agent search optimization, behavioral
  hardening, skill types). See the skill's attribution section for details.
